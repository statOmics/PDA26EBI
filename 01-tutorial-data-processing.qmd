...
[1.a] Participants can perform an analysis using the [Rmarkdown script](./staes-medianOfRatios-maxLFQ.html) or the [msqrob2GUI](./staes248-GUI.html). 
...
centers all samples (columns) so that they all match the grand median by subtracting the respective columns medians differences to the grand median (code: `qf <- normalize(qf,   i = "precursors_log", name = "precursors_norm", method = "diff.median")`.  What do you observe and try to explain this.
...
Users that work with scripts can modify the Rmarkdown script](./cptac.html). 
